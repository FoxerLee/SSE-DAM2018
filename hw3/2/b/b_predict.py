...
def main():
    ...
    overall = '567'
    label_month = '08'
    test_datas = util.train_generator(months, overall, label_month, predict=True)
    y_test = test_datas.index.tolist()
    X_test = test_datas.drop(columns=['vipno', 'pluno'], inplace=False).as_matrix()

    smote_enn = SMOTEENN(random_state=0)
    X_train, y_train = smote_enn.fit_sample(X_train, y_train)

    # 高斯朴素贝叶斯分类器
    f = open('predict/1552674_2b_GaussianNB.txt', 'w')
    clf = GaussianNB()
    ...
    # K近邻分类器
    f = open('predict/1552674_2b_KNeighborsClassifier.txt', 'w')
    ...
    # 决策树分类器
    f = open('predict/1552674_2b_DecisionTreeClassifier.txt', 'w')
    ...
    # 随机森林
    f = open('predict/1552674_2b_RandomForestClassifier.txt', 'w')
    ...
    # AdaBoost
    f = open('predict/1552674_2b_AdaBoostClassifier.txt', 'w')
    ...
    # Bagging
    f = open('predict/1552674_2b_BaggingClassifier.txt', 'w')
    ...
    # GradientBoosting
    f = open('predict/1552674_2b_GradientBoostingClassifier.txt', 'w')
